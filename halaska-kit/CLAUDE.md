...
context-bar (ContextBarPattern — a floating pill bar, like the site's own action bar, whose three suggestions change with the on-screen context: Thread / Doc / Calendar; autoplays until touched; "Alpha is on it" state), space-deck (SpaceDeckPattern — a card deck of agents; horizontal swipe/keys between agents, vertical swipe/keys or the header pill between spaces Studio and Personal, each with its own accent; Gmail-on-mobile style). Prefixes CTXBAR_ / SPACEDECK_.
