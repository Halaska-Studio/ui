...
`api/submit.js` is a Vercel serverless function for the studio review form; it emails via Resend (`RESEND_API_KEY`) or forwards to `SUBMIT_WEBHOOK_URL`, and returns 503 when neither is set. `vite.config.js` shims `/api/submit` in dev (logs to the terminal). `src/main.jsx` renders a single piece when `?shot=after` or `?shot=before-after` is in the URL, for the README screenshots (headless Chrome, see the pre-launch PR).
