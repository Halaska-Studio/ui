...
**Security posture (2026-09-10 pass):** no secrets in the working tree, the deployed files, or git history (the baseline commit's `.claude/settings.local.json` and zip were purged from history with filter-branch and force-pushed). GitHub secret scanning and push protection are enabled on the repo. `.claude/`, `.vercel/`, `.env*`, zips and generated `public/` files are ignored. The only email in the repo is the intended form recipient (chris@halaska.com) in `api/submit.js` and the README; it can move to the `SUBMIT_TO` env var if it should not be public. Delivery credentials live only in Vercel env vars (none set yet).

`api/submit.js` is a Vercel serverless function for the Feedback modal (`kind: "feedback"`, text + optional email) and the studio review form (`kind: "review"`); it emails via Resend (`RESEND_API_KEY`) or forwards to `SUBMIT_WEBHOOK_URL`, and returns 503 when neither is set. No delivery is configured yet (Chris removed the Zapier hook entirely on 2026-09-10; he doesn't use that Zap). Until `RESEND_API_KEY` (or another webhook) is set on Vercel, both forms show the inline "Couldn't send" error in production. `vite.config.js` shims `/api/submit` in dev (logs to the terminal). `src/main.jsx` renders a single piece when `?shot=after` or `?shot=before-after` is in the URL, for the README screenshots (headless Chrome, see the pre-launch PR).
